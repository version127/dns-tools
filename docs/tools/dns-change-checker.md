...
A different resolver answer is evidence of a different cached value, not proof of worldwide propagation or a broken resolver. When authoritative replies differ, the record set returned by the largest number of usable authoritative sources becomes the resolver comparison baseline. The first server to answer never gets special weight. Resolver agreement counts only usable replies; timeouts and unavailable resolvers are stated separately. Location-aware A and AAAA answers can also differ legitimately.
