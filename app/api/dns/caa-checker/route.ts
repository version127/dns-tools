...
import { checkCaaPolicy } from "@/lib/dns/diagnostics.ts";
...
export async function POST(request: Request) {
  ...
  const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), 20_000); request.signal.addEventListener("abort", () => controller.abort(), { once: true });
  try { return dnsJson(await checkCaaPolicy(body.name, { signal: controller.signal }), Math.min(limit.remainingRequests, limit.remainingUnits)); }
  catch (error) { return dnsErrorResponse(controller.signal.aborted ? 504 : 400, controller.signal.aborted ? "check_timeout" : "check_failed", controller.signal.aborted ? "The CAA check took too long and was stopped safely." : error instanceof Error ? error.message : "The CAA check could not be completed."); }
